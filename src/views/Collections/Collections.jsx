...
const Collections = () => {
  ...
  useEffect(() => {
    const getSaleNFTs = async () => {
      const data = await getAllListedNFT();
      if (data) {
        let dataArray = Object.values(data);
        setNftsSaleData(dataArray);
      }
    };
    getSaleNFTs();
  ...
  // console.log("nftsSaleData", nftsSaleData);
  ...
  //   ...new Map(userNfts.map((nfts) => [nfts[key], nfts])).values(),
  // ];
  const nftsCollection = [
    ...new Map(
      nftsSaleData?.map(
        (data) =>
          Object.keys(data).map((keyName, i) => [data[keyName], data[keyName]])
        // data[keyName])
        ...
        <CustomContainer>
          <Grid container spacing={3}>
            ...
            {/* {nftsCollection?.map((nft) =>
              nft?.map((obj) => (
                <Grid item xs={12} sm={6} md={6} lg={4} key={obj.tokenID}>
                  <CollectionDetailCard data={obj} />
                </Grid>
              ))
            )} */}
            {nftsSaleData?.map((data) =>
              Object.keys(data).map((keyName, i) => (
                <Grid
                  ...
                  md={6}
                  lg={4}
                  key={data[keyName].tokenID}
                >
                  <CollectionDetailCard data={data[keyName]} />
                </Grid>
              ))
            )}
            <Grid item xs={12}>
